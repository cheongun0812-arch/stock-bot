...
st.markdown("""
    ...
    .disclaimer { font-size: 0.9rem; color: #666666; text-align: center; margin-bottom: 30px; line-height: 1.6; background-color: #fdf2f2; padding: 10px; border-radius: 8px; }
    ...
    if raw_input.isdigit() and len(raw_input) == 6:
        for suffix in [".KS", ".KQ"]:
            ...
            if not hist.empty:
                ...
                # 한글 종목명 매핑 강화
                mapping = {"Samsung Electronics Co., Ltd.": "삼성전자", "SK hynix Inc.": "SK하이닉스", "LG Energy Solution, Ltd.": "LG에너지솔루션"}
...
# --- [4. 메인 화면: 한글명 및 법적 주의문구 복구] ---
st.markdown(f"<div class='main-title'>📈 {s_name} 투자 시뮬레이션</div>", unsafe_allow_html=True)
st.markdown(f"<div class='disclaimer'>⚠️ <b>주의:</b> 본 프로그램은 개인적인 투자 전략 수립을 돕기 위한 <b>참고용 시뮬레이터</b>입니다. <br> 제공되는 모든 데이터와 계산 결과는 실제 거래 결과와 다를 수 있으며, 투자에 대한 모든 판단과 <b>법적 책임은 투자자 본인</b>에게 있습니다.</div>", unsafe_allow_html=True)
...
with st.expander("데이터 입력 (입력값 유지)", expanded=True):
    ...
    # 아빠가 입력한 값 보존
    current_avg = c1.number_input(f"현재 내 평단가", value=st.session_state.my_avg if st.session_state.my_avg > 0 else float(live_p), step=100.0 if market=="KR" else 0.01)
    current_qty = c2.number_input("현재 보유 수량 (주)", value=st.session_state.my_qty, step=1)
    now_p = c3.number_input(f"현재 시장가 (실시간)", value=float(live_p), step=100.0 if market=="KR" else 0.01)
...
if total_qty > 0:
    ...
    status = "상승" if avg_diff > 0 else "하락"
    st.markdown(f"""
...
chart = get_technical_chart(ticker)
