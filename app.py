...
st.markdown("""
    ...
    .disclaimer { font-size: 0.9rem; color: #666666; text-align: center; margin-bottom: 30px; line-height: 1.5; }
...
def get_advanced_chart(ticker_symbol):
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['MA5'], line=dict(color='#FFD700', width=1), name='5일선'))
    fig.add_trace(go.Scatter(x=df.index, y=df['MA20'], line=dict(color='#FF1493', width=1.2), name='20일선'))
    fig.add_trace(go.Scatter(x=df.index, y=df['MA60'], line=dict(color='#00BFFF', width=1.5), name='60일선'))
    fig.add_trace(go.Scatter(x=df.index, y=df['MA120'], line=dict(color='#8B4513', width=2), name='120일선'))
    fig.update_layout(title=f"최근 1년 주가 흐름 및 이동평균선", yaxis_title="가격", xaxis_rangeslider_visible=False, height=500, template="plotly_white", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig
...
with st.sidebar:
    ...
    ex_rate = (yf.Ticker("USDKRW=X").history(period="1d")['Close'].iloc[-1]) if market == "US" else 1380.0
    if ticker:
...
# [요청 사항] 면책 조항 안내 문구 추가
st.markdown(f"<div class='disclaimer'>본 프로그램에서 제공하는 모든 수치 및 분석 결과는 단순 참고용이며, 투자 권유를 목적으로 하지 않습니다.<br>모든 투자 의사결정의 책임은 투자자 본인에게 있으며, 본 시스템은 결과에 대한 법적 책임을 지지 않습니다.</div>", unsafe_allow_html=True)
...
if total_qty > 0:
    ...
    st.markdown(f"<div class='result-summary'>☞ 본 시뮬레이션 분석 결과 주당 평단가 <span style='color:{d_color};'>{d_sign} {abs(avg_diff):,.2f} {d_word}</span>이 되었습니다.</div>", unsafe_allow_html=True)
...
df_res = pd.DataFrame({
    ...
    "매수 후 예상": [f"{total_qty:,}주", f"{final_avg:,.2f}", f"{aft_profit:+,.0f}", f"{aft_rtn:.2f}%"]
}).set_index("항목")
...
st.write("본 가이드는 AI 알고리즘에 기반한 기술적 분석 데이터이며, 실제 시장 상황과 다를 수 있음을 유의하시기 바랍니다.")
