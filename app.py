...
# --- [4. 메인 화면: 한글명 및 주의문구 복구] ---
st.markdown(f"<div class='main-title'>📈 {s_name} 투자 시뮬레이션</div>", unsafe_allow_html=True)
st.markdown("<div class='disclaimer'>본 프로그램에서 제공하는 정보는 참고용이며, 투자 권유를 목적으로 하지 않습니다.<br>모든 투자 판단의 책임은 투자자 본인에게 있으며, 결과에 대한 법적 책임을 지지 않습니다.</div>", unsafe_allow_html=True)

# 1️⃣ 내 현재 보유 현황 (입력값 고정)
st.markdown(f"<div class='section-title'>👤 1️⃣ 내 현재 보유 현황 ({unit})</div>", unsafe_allow_html=True)
with st.expander("데이터 입력 (입력한 값은 고정됩니다)", expanded=True):
...
if total_qty > 0:
    ...
    st.markdown(f"<div class='result-summary'>☞ 분석 결과: 추가 매수 시 예상 평단가는 기존 대비 <span style='color:{color};'>{abs(avg_diff):,.2f} {unit} {status}</span>합니다.</div>", unsafe_allow_html=True)
